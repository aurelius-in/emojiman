...
// New element for displaying lives
...
livesElement.style.fontSize = '30px';
livesElement.style.fontFamily = "'Press Start 2P', cursive";
...
function updateLives() {
    ...
    livesElement.textContent = livesStr;
}

// Call this function to update lives display initially
updateLives();
...
function checkEnemyCollision() {
    for (let i = 0; i < enemies.length; i++) {
        if (pacman.x<enemies[i].x + enemies[i].width &&
pacman.x + pacman.width> enemies[i].x && pacman.y<enemies[i].y + enemies[i].height &&
pacman.y + pacman.height> enemies[i].y) {

            if (powerMode) { // Remove enemy
                enemies.splice(i, 1);
                score += 10;
            } else { // Pacman loses a life
                lives--;
                if (lives > 0) { // Update lives display
                    let livesDisplay = "";
                    for (let j = 0; j < lives; j++) {
                        livesDisplay += "😃";
                    }
                    document.getElementById("lives").innerText = livesDisplay;

                    // Reset pacman and enemies position
                    ...
                    // Show 😩 emoji
                    ctx.font = "30px Arial";
                    ctx.fillText("😩", pacman.x, pacman.y);
                } else { // Game over, reset everything
                    resetGame();
                }
            }
        }
...
function resetEnemies() { // Assuming enemies is an array of enemy objects
    enemies.forEach(enemy => {
...
function resetPellets() { // Assuming pellets is an array of pellet objects
    pellets.forEach(pellet => {
        pellet.active = true; // Reactivate the pellet
    });
}

function resetGame() { // Logic to reset the game, including pellets and lives
    lives = 3;
...
function enemyBehavior(enemy) {
    ...
    if (currentTime - enemy.lastMove < 500) 
        return;
...
if (maze[nextGridY][nextGridX] !== '1' || currentTime>enemy.smarterTime
    ...
    function gameLoop() {
        ...
        const pacmanColor = isPowerMode ? 'white' : 'yellow';
